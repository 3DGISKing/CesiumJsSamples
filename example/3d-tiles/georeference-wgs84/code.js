const { Cartesian3, Cartographic, Cesium3DTileset, Color, createWorldTerrainAsync, DebugModelMatrixPrimitive, HeadingPitchRoll, knockout, Matrix3, Matrix4, Model, Quaternion, Transforms } =
    window.Cesium;
...
function updateModelMatrixOfTilesetDefinedAtECEF(options) {
    ...
    centerOfBoundingSphereDebugModelMatrixPrimitive.modelMatrix = Transforms.eastNorthUpToFixedFrame(tileset.boundingSphere.center);
    origCenterDebugModelMatrixPrimitive.modelMatrix = Transforms.eastNorthUpToFixedFrame(origBoundingBoxCenter);
    geoRefDebugModelMatrixPrimitive.modelMatrix = Transforms.eastNorthUpToFixedFrame(new Cartesian3(position.x + x, position.y + y, position.z + z));
}
...
async function main() {
    ...
    const tilesetPromise = Cesium3DTileset.fromUrl("https://s3.us-east-2.wasabisys.com/construkted-assets/au0x0as79k3/tileset.json");
    ...
    tilesetPromise
        .then((t) => {
            ...
            const length = t.boundingSphere.radius;
            ...
            viewer.camera.flyToBoundingSphere(t.boundingSphere);
        })
        .catch((e) => {
            console.error(e);
        });
